	...
	std::vector<double> hVec)
{
    ...
    itensor::MPO Liouv = toMPO(LiouvAMPO);
    itensor::MPO LiouvD = toMPO(LiouvDAMPO);

    itensor::MPO LdL = itensor::nmultMPO(Liouv, prime(LiouvD),  {"MaxDim", 50000,"Cutoff",1E-8}); // constructs Ld*L
    LdL.mapPrime(2,1); // un-prime LdL after multiplying
    //print("LdL = ", LdL);
